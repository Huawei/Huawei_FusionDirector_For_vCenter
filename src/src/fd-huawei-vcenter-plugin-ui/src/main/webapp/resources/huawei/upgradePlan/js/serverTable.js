Vue.component('serverTable', {
    template: '<el-table ref="servertable" v-bind:data="data" style="width:100%;" header-row-class-name="my_table_header" @expand-change="toggleRow" @select="select" @select-all="selectAll" :row-class-name="getClassName">'
            ...
            +           '<el-table-column  :label="i18ns.deviceVersionStatus.deviceVersionStatus_deviceName"> </el-table-column>'
            +           '<el-table-column  :label="i18ns.deviceVersionStatus.deviceVersionStatus_model"> </el-table-column>'
            ...
            +           '<el-table-column prop="CurrentDriverVersion" :label="i18ns.deviceVersionStatus.deviceVersionStatus_currentDriverVersion" :show-overflow-tooltip="true"> </el-table-column>'
            +           '<el-table-column prop="TargetDriverversion" :label="i18ns.deviceVersionStatus.deviceVersionStatus_targetDriverVersion" :show-overflow-tooltip="true"> </el-table-column>'
            +       '</el-table>'
            +   '</template>'
            +'</el-table-column>'
            ...
            +'</el-table-column>'
            +'<el-table-column prop="CurrentDriverVersion" :label="i18ns.deviceVersionStatus.deviceVersionStatus_currentDriverVersion" :show-overflow-tooltip="true">'
            +    '<template slot-scope="scope"> {{scope.row.Firmwares&&scope.row.Firmwares.length<=1?scope.row.Firmwares[0].CurrentDriverVersion:""}} </template>'
            +'</el-table-column>'
            +'<el-table-column prop="TargetDriverversion" :label="i18ns.deviceVersionStatus.deviceVersionStatus_targetDriverVersion" :show-overflow-tooltip="true">'
            +   '<template slot-scope="scope"> {{scope.row.Firmwares&&scope.row.Firmwares.length<=1?scope.row.Firmwares[0].TargetDriverversion:""}} </template>'
            +'</el-table-column>'
            +'</el-table>',
    data: function () {
        return {
    ...
    created: function () {
        this.i18ns = getIn18();
        this.data =  JSON.parse(JSON.stringify(this.serverData))
    },
    updated:function(){
        var _this = this;
        this.$nextTick(function () {
            _this.data.forEach(function(row) {
                if(row.Selected==true){
                    _this.$refs.servertable.toggleRowSelection(row,true);
                }else{
                    _this.$refs.servertable.toggleRowSelection(row,false);
                }
            })
        })
    },
    methods: {
        handleClick:function(){
            var _this =this
            this.$nextTick(function () {
                _this.$refs['servertable'].doLayout()
            })
        },
        hideFirmwares:function(){
            var _this =this
            this.data.forEach(function(row) {
                _this.$refs.servertable.toggleRowExpansion(row,false)
            })
        },
                ...
                if(item.Index==row.Index){
                        ...
                        if(group.indexOf(dataItem.Index)>-1&&dataItem.Selected==false){
                            (function(arg){
                                Vue.nextTick(function () {
                                    _this.$refs.servertable.toggleRowSelection(arg,true);
                                })
                            })(dataItem);
                            dataItem.Selected=true;
                        }
                    ...
                    data.forEach(function(dataItem) {
                        ...
                        if(group.indexOf(dataItem.Index)>-1&&dataItem.Selected==true){
                            (function(arg){
                                Vue.nextTick(function () {
                                    _this.$refs.servertable.toggleRowSelection(arg,false);
                                })
                            })(dataItem);
                            dataItem.Selected=false;
                        }
        ...
        toggleRow:function(row, expandedRows){
            if(!row.Firmwares||row.Firmwares.length<=1){
                this.$refs.servertable.toggleRowExpansion(row,false)
            }
